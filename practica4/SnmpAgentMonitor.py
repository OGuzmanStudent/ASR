...
class SNMPAgent:
    ...
    def generate_file_by_telnet(self):
        with Telnet(self.agent_ip_address) as telnet_connection:
            telnet_connection.read_until(b"login: ")
            telnet_connection.write("rcp".encode('ascii') + b"\n")
            telnet_connection.read_until(b"Password: ")
            telnet_connection.write("rcp".encode('ascii') + b"\n")
            telnet_connection.write(b"enable\n")
            telnet_connection.write(b"configure\n")
            telnet_connection.write(b"copy run start\n")

    def get_configuration_by_ftp(self):
        ftp = FTP(self.agent_ip_address)
        ftp.login()
        with open(f'{self.agent_ip_address}-startup-config', 'wb') as fp:
            ftp.retrbinary('startup-config', fp.write)
        ftp.quit()

    def set_configuration_by_ftp(self, configuration_filename):
        ftp = FTP(self.agent_ip_address)
        ftp.login()
        with open(configuration_filename, 'rb') as fp:
            ftp.storbinary('startup-config', fp)
        ftp.quit()
